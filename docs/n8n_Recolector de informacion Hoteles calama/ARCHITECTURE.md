...
|-----------|--------|--------|-------------|
| Workflow A | `QBDVpsKWGTHZZikE` | Activo (MODO TEST) | Scraping semanal — Loop 21 iteraciones (7 días × 3 tipos) |
| Subworkflow | `YXDogRLJrprijeiV` | Activo | Análisis IA + email (21 nodos) |
| Workflow B | `SbI0cfUlQvtLWvxZ` | Activo | Disparador manual (Form Trigger + Execute Workflow A) |
| Workflow C | pendiente | — | Agente consultor vía Slack DM |
| Workflow [UTIL] | `yyaxraPUve3ApgGl` | Activo | Postear botón Slack en #ventas-atankalama |
...
```
Cron: 0 10 * * 1  (Lunes 10:00 UTC = 6:00 AM Santiago)
MODO TEST: filtra solo 2 hoteles (atankalama + geotel-calama) en Code — Build Apify input
Para producción: quitar .filter() en ese nodo
```
...
### Pendiente
- Verificar que `price_snapshots` acepta los campos `html`, `analysis`, `scrape_date` (autoMapInputData puede fallar si columnas difieren)
...
## Workflow C — Agente Slack DM (planificado)

**Prerrequisitos**: credencial `anthropicApi` (ya existe), escopos Slack App (`im:history`, `im:read`, `im:write`, `chat:write`, `message.im`)

```
[Slack Trigger (DM)] → [IF anti-loop (no bot_id)] → [AI Agent claude-sonnet-4-6]
  Tools:
  - Tool Code: consultar_precios_puntuales → hotel_prices
  - Tool Code: consultar_tendencia_historica → hotel_prices (N semanas)
  - Tool HTTP: get_ultimo_snapshot → price_snapshots ORDER BY date DESC LIMIT 1
→ [Slack — Responder DM]
```
...
| Gmail | `tRN6oegZdk974irw` | OAuth2 zurk.calameno@gmail.com |
